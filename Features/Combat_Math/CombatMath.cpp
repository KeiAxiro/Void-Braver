...
Equipment getEquipmentData(string name) {
    ...
    if (name == "Void-Iron Cleaver") return {"Void-Iron Cleaver", "Weapon", 610, 0, 0};
    if (name == "Rusty Greatsword") return {"Rusty Greatsword", "Weapon", 15, 0, 0};
    // HELMET
    if (name == "Marshall's Iron Face") return {"Marshall's Iron Face", "Helmet", 0, 1500, 8000};
    if (name == "Iron Pot Helm") return {"Iron Pot Helm", "Helmet", 0, 50, 250};
    // ARMOR
    if (name == "God-King's Bastion") return {"God-King's Bastion", "Armor", 0, 3000, 15000};
    if (name == "Knight's Plate") return {"Knight's Plate", "Armor", 0, 300, 1500};
    if (name == "Leather Jerkin") return {"Leather Jerkin", "Armor", 0, 50, 300};
    // BOOTS
    if (name == "Grand Marshall's Stride") return {"Grand Marshall's Stride", "Boots", 0, 1500, 8000};
    if (name == "Traveler's Boots") return {"Traveler's Boots", "Boots", 0, 25, 100};
    // ACCESSORY
...
void showEquipmentMenu(Player& p) {
    ...
    do {
        ...
        cout << left << "  " << setw(26) << "1. Pasang / Ubah" << endl << "  " << "2. Kembali" << endl;
