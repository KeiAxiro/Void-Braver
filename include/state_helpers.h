...
// Helper internal untuk modul state.
// Header ini dipakai antar file di src/states, sedangkan states.h tetap menjadi
// API publik yang kecil: runMainMenu, runHub, dan runBattle.
namespace state_helpers
{
    ...
    inline constexpr int kStandardBossDepth = 20;
    inline constexpr int kFinalDungeonBossSpawnDepth = 21;
    ...
    bool isUsableBossName(const std::string &name);
    std::string normalizeBossName(std::string value);
    bool bossNameMatches(const std::string &left, const std::string &right);
    bool textMentionsBossName(const std::string &text, const std::string &bossName);
    bool enemySpawnDepthMatches(const json &enemy, int depth);
    std::string findBossNameInText(const GameContext &ctx, const std::string &text);
    std::string bossNameForDepth(const GameContext &ctx, const json &dungeon, int depth);
    void addBossesByNameToPool(const GameContext &ctx, JsonPointerList &pool, const std::string &bossName);
