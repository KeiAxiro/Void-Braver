...
namespace consoleui
{
    ...
    // Memainkan dialog depth satu baris per baris.
    // dialogLines adalah array JSON: [{ "speaker": "...", "text": "..." }, ...]
    // Jika array kosong atau null, fungsi langsung return tanpa output.
    inline void playDialog(const nlohmann::json &dialogLines)
    {
        if (!dialogLines.is_array() || dialogLines.empty())
            return;
        ...
        for (const auto &line : dialogLines)
        {
            ...
            const std::string speaker = line.value("speaker", std::string(""));
